...
function getRandomCoffeeMatch() {
  const matches = [
    "Sarah Johnson from Sustainability",
    "Mike Chen from Data Analytics",
    "Jennifer Williams from Customer Experience",
    "David Rodriguez from Innovation Lab",
    "Lisa Park from Operations",
  ];
...
function handleCoffeeMatch() {
  const match = getRandomCoffeeMatch();
  appState.coffeeMatch = match;

  const resultContainer = document.getElementById("coffeeMatchResult");
  if (resultContainer) {
    resultContainer.innerHTML = `
      <div class="coffee-match-info">
        <div class="coffee-match-avatar">${getInitials(match.split(" from")[0])}</div>
        <div class="coffee-match-text">
          <div class="coffee-match-label">You've been matched with:</div>
          <div class="coffee-match-name">${match}</div>
        </div>
      </div>
      <button class="btn btn-primary" style="margin-top: 0.75rem;">Schedule Meeting</button>
    `;
    resultContainer.classList.remove("hidden");
  }

  const findBtn = document.getElementById("findCoffeePartnerBtn");
  if (findBtn) {
...
function setupEventListeners() {
  ...
  const findCoffeeBtn = document.getElementById("findCoffeePartnerBtn");
  if (findCoffeeBtn) {
    findCoffeeBtn.addEventListener("click", handleCoffeeMatch);
  }
